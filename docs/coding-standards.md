...
## TypeScript/Node.js
- `*.ts` と `*.tsx` の各ファイルごとに `npm run tsdoc-extract -- <relative-path>` コマンドを実行し、その出力について以下を確認する
...
- モジュール内でしか使わないユーティリティ関数は export しない。公開API（export する関数・型）を先に定義し、非公開ユーティリティはその後方に配置する。
    - 必要があって別ファイルへ切り出す場合でも同じディレクトリに置いて距離を縮め、後から削除しやすい構成にすること
- サプライチェーン攻撃に備えて以下を徹底する
    - package.jsonのバージョン指定で `^` を使わないこと
- テストを追加する際は、実装と同じディレクトリに `*.test.ts` / `*.test.tsx` を置き、Vitest (`describe`/`it`/`expect`) ベースでテストを書く
